  ...
  console.error('Fetch error:', error)
   ...
   const totalPrice = data.reduce((preve,curr)=>preve + (curr.quantity * curr?.productId?.sellingPrice),0)
    ...
    <div className='container mx-auto'>
      ...
      <div className='flex flex-cols lg:flex-row gap-10 lg:justify-between p-4'>
            {/* view product */}
            <div className='w-full max-w-3xl'>
              {
                loading ? (
                    ...
                    data.map((product,index)=>{
                        return(
                            <div key={product?._id+"Add To Cart Loading"} className='w-full bg-white h-32 my-2 border border-slate-300 rounded grid grid-cols-[120px,1fr] '>
                                <div className='w-32 h-32 bg-slate-200'>
                                    <img src={product?.productId?.productImage[0]} className='w-full h-full object-scale-down mix-blend-multiply' />
                                </div>
                                <div className='px-4 py-2 relative'>
                                    {/* delete product */}
                                    <div className='absolute right-0 text-red-600 rounded p-2 hover:text-white hover:bg-red-600 cursor-pointer' onClick={()=>deleteCartProduct(product?._id)}>
                                    <MdDelete />
                                    </div>
          ...
          <div className='mt-5 lg:mt-0 w-full max-w-sm'>
          {
            loading ? (
                ...
                <div className='h-36 bg-white'>
                 ...
                 <button className='bg-blue-600 py-2 mt-4 text-white w-full'>Payment</button>
            </div>
            )
        }
          </div>
