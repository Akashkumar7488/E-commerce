...
const UploadProduct = (
    ...
    // product model for displaying frontend side
  ...
  return (
    <div className='fixed bg-slate-200 bg-opacity-35 w-full h-full  bottom-0 top-0 left-0 right-0 flex justify-center items-center '>
      <div className='bg-white p-4 rounded w-full max-w-2xl h-full max-h-[80%] overflow-hidden'>
        ...
        <form className='grid p-4 gap-3 overflow-y-scroll h-full pb-5' onSubmit={handleSubmit}>
            ...
            <select  required value={data.category} name='category' onChange={handleOnChange} className='p-2 bg-slate-100 border rounded cursor-pointer' >
                        ...
                        <div className='flex items-center gap-2'>
                            {
                                data.productImage.map((el,index)=>{
                                    return(
                                        ...
                                        <div className='absolute bottom-0 right-0 p-0.5 text-white bg-red-600 rounded hidden group-hover:block cursor-pointer' onClick={() => handleDeleteProductImage(index)}>
                                        <MdDelete />
                                        </div>
                                        </div>
                                    )
           ...
           <label htmlFor="sellingPrice" className='mt-3'>Selling Price:</label>
            ...
            placeholder='enter sellingPrice' value={data.sellingPrice} 
            onChange={handleOnChange}  
            className='p-2 bg-slate-100 border rounded '  required/>

            <label htmlFor="description" className='mt-3'>Description:</label>
            <textarea className='h-28 bg-slate-100 border resize-none p-1' placeholder='enter product description' rows={3} cols={5}   onChange={handleOnChange} name='description'>
                </textarea>  
